...
void drawChar(char c, uint8_t x, uint8_t y, uint8_t color) {
    if (c < 32 || c > 127) return; // Only printable
    
    // Draw 8x8 character using standard font
    // Note: drawBitmap expects the input format to match the hardware's row/column addressing
    // and packing. The generic drawBitmap might need adjustment or we can write a specific one.
    // Given drawBitmap logic:
    // It takes bytes, and for each byte iterates 8 bits.
    // It writes 4 bits per pixel (presumably 4bpp mode).
    // And it assumes the bitmap data is linear.
    
    // For 8x8 characters, we have 8 bytes of data (rows).
    // We can use drawBitmap directly if we pass the address of the char in font array.
    
    drawBitmap((uint8_t*)font8x8_basic[c - 32], x, y, 8, 8, color);
}

void drawString(const char* str, uint8_t x, uint8_t y, uint8_t color) {
    while (*str) {
        drawChar(*str, x, y, color);
        x += 8; // Move to next char position (8 pixels wide)
        if (x > DISPLAY_WIDTH - 8) break; // Simple clipping
        str++;
    }
}

void drawCharScaled(char c, uint8_t x, uint8_t y, uint8_t color, uint8_t scale) {
    ...
    uint8_t height = 8 * scale;
    
    clrBuff(width, height);
    setDisplayWindow(x, y, width, height);
    /* 
       Note: The original drawBitmap function sets the window and then writes to memory.
       The memory layout seems to be linear in terms of 32-bit words.
       Each 32-bit word holds 8 pixels (4 bits per pixel).
       The packing is: MSB (pixel 0) at bits 28-31, LSB (pixel 7) at bits 0-3.
    */

    uint16_t u32Idx = 1;
    uint32_t u32Pixels = 0;
    uint8_t pixelInWord = 0; 

    // Font data pointer
    const uint8_t *charData = (const uint8_t*)font8x8_basic[c - 32];

    // For each row of the character
    for (int row = 0; row < 8; row++) {
        uint8_t rowData = charData[row];
        
        // Repeat this row 'scale' times vertically
        for (int s_y = 0; s_y < scale; s_y++) {
            
            // For each bit in the row (column)
            for (int col = 0; col < 8; col++) {
                // Check bit at 'col' (MSB first: bit 7 down to 0)
                // Actually font is: MSB is left-most pixel. 
                // bit 7 is pixel 0.
                bool pixelOn = (rowData & (1 << (7 - col))) != 0;
                
                // Repeat this pixel 'scale' times horizontally
                for (int s_x = 0; s_x < scale; s_x++) {
                    if (pixelOn) {
                        // Pixel position in word (0 to 7)
                        // 0 means left-most, corresponds to shift 28.
                        // 7 means right-most, corresponds to shift 0.
                        uint8_t shift = (7 - pixelInWord) * 4;
                        u32Pixels |= (uint32_t)color << shift;
                    }
    ...
    // Flush any remaining pixels (should not happen if width is multiple of 8)
    if (pixelInWord > 0) {
        write32ToMemory(u32Idx++, u32Pixels);
    }
...
void drawStringScaled(const char* str, uint8_t x, uint8_t y, uint8_t color, uint8_t scale) {
    while (*str) {
        drawCharScaled(*str, x, y, color, scale);
        x += (8 * scale); // Move to next char position
        if (x > DISPLAY_WIDTH - (8 * scale)) break; // Simple clipping
        str++;
    }
}
...
uint8_t drawBitmap(uint8_t *img, uint8_t x, uint8_t y, uint8_t width, uint8_t height, uint8_t color) {
    ...
    for (int i = 0; i < pixel2Draw; i++) {
        ...
        i += 7; // This skips ahead because we processed 8 pixels in one byte iteration. 
                // But loop condition is i < pixel2Draw, and i increments by loop as well?
                // Wait, 'for (int i = 0; i < pixel2Draw; i++)'. 
                // If I modify 'i' inside, loop increments again.
                // Original code: i += 7. So next iteration i is +8.
                // 64/8 = 8 iterations. Correct.
